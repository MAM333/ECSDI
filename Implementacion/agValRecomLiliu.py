...
def comprobar_y_valorar():
    ...
    if to_rate:
        ...
        message = build_message(grafoContenido, 
                               perf=ACL.request, 
...
def start_scheduler():
    ...
    ""
    scheduler.add_job(recommending, 'interval', minutes=10)
    scheduler.add_job(generate_recommendations, 'interval', minutes=10)
    scheduler.start()
...
def recomendar():
    logger.info("Iniciando peticion rutinaria de valoraciones")
    threading.Thread(target=recommending).start()
    sleep(60*10)
    recomendar()
