...
# Datos del Agente
AgProductosExternos = Agent('AgProductosExternos',
                  agn.AgProductosExternos,
...
def register_message():
    ...
    gr = registerAgent(AgProductosExternos, DirectoryAgent, AgProductosExternos.uri, getMessagesCount())
    return gr
...
def comunicacion():
    ...
    if messageProperties is None:
        ...
        resultadoComunicacion = build_message(
            Graph(), ACL['not-understood'], sender=AgProductosExternos.uri, msgcnt=getMessagesCount())
    else:
        ...
        if perf != ACL.request:
            ...
            if accion == ECSDI.product:
                resultadoComunicacion = anyadirProducto(grafoEntrada)

    serialize = resultadoComunicacion.serialize(format='xml')
    return serialize

def anyadirProducto(grafoEntrada):

    logger.info("Añadiendo producto externo a la BD")
    ...
    grafoProductos += grafoEntrada

    grafoProductos.serialize(destination='./database_producto.rdf', format="xml")
